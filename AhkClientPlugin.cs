...
namespace AhkClient
{
    public class AhkClientPlugin
    {
        ...
        static AhkClientPlugin()
        {
            Type[] types = typeof(AhkClientPlugin).Assembly.GetTypes();
        ...
        public async Task RunAsync(int port, string uuid, string registerEvent)
        {
            ...
            m_Connection.OnDidReceiveGlobalSettings += Connection_OnDidReceiveGlobalSettings;
        ...
        private async void Connection_OnDidReceiveGlobalSettings(object sender, StreamDeckEventReceivedEventArgs<DidReceiveGlobalSettingsEvent> e)
        {
            ...
            JObject globalSettings = e.Event.Payload.Settings;
            try
            {
                foreach (KeyValuePair<string, ActionBase> entry in m_Actions)
                {
                    await entry.Value.LoadGlobalSettings(globalSettings.ToObject<AhkGlobalSettingsDef>());
                }
            }
            finally
            {
            ...
            //MessageBox.Show(globalSettings.ToString(), "title");
        }
